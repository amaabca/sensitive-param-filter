import { WhiteListRegex } from './interfaces'
...
const constructParamRegex = (params: string[]): RegExp => {
...
const constructWhitelistRegex = (whitelist: string[]): RegExp | WhiteListRegex => {
  if (whitelist && whitelist.length) {
    const whitelistRegexGroups = whitelist.map((entry: string) => `^(${entry})$`)
    return new RegExp(whitelistRegexGroups.join('|'))
  }
  return { test: (): boolean => false }
}

const generateRandomString = (): string => randomBytes(32).toString('hex')
...
const parseUrlParams = (input: string): Segments[] => {
  const segments = []
